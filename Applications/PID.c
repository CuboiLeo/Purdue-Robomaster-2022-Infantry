...
*/
//********************云台yaw轴参数********************
float PID_YAW_SPEED_KP = 10;
float PID_YAW_SPEED_KI = 0.01;
float PID_YAW_SPEED_KD = 0;
float PID_YAW_ANGLE_KP = 100;
float PID_YAW_ANGLE_KI = 0;
float PID_YAW_ANGLE_KD = 0;


//*******************云台pitch轴参数*******************
float PID_PITCH_SPEED_KP = 5;
float PID_PITCH_SPEED_KI = 0.01;
float PID_PITCH_SPEED_KD = 0;
float PID_PITCH_ANGLE_KP = 80;
float PID_PITCH_ANGLE_KI = 0;
float PID_PITCH_ANGLE_KD = 0;
...
PID_Controler Fric_Wheel_pid[2];
...
void Total_PID_Init(void)
{
  ...
  //Pitch_speed_pid.Integrate_Limit_Flag = 1;//积分限幅
	...
	//Pitch_speed_pid.Integrate_Max = 3000;//积分幅值
	...
	Pitch_angle_pid.Control_OutPut_Limit = 2000;//最大输出
	
	
	//********************云台yaw轴********************
	//Yaw_speed_pid.Integrate_Limit_Flag = 1;//积分限幅
	//Yaw_speed_pid.Integrate_Max = 3000;
	...
	for(i=0;i<2;i++)
	{
		Fric_Wheel_pid[i].Integrate_Limit_Flag = 1;
		Fric_Wheel_pid[i].Integrate_Max = PID_FRIC_SPEED_I_MAX;
		Fric_Wheel_pid[i].Kp = PID_FRIC_SPEED_KP;
		Fric_Wheel_pid[i].Ki = PID_FRIC_SPEED_KI;
		Fric_Wheel_pid[i].Kd = PID_FRIC_SPEED_KD;
		Fric_Wheel_pid[i].Control_OutPut_Limit = 3000;
		Fric_Wheel_pid[i].Integrate_Separation_Err = PID_FRIC_SPEED_I_ERROR;
	}
...
float PID_Control(PID_Controler *Controler,float target,float measure)
{
	...
	Controler->Err = target - measure;
	if((Controler->Err_Max != 0) && (fabs(Controler->Err) > Controler->Err_Max))
	{
		return 0;
	}
	Controler->Pout = Controler->Kp * Controler->Err;
	Controler->Iout += Controler->Ki * Controler->Err;
	Controler->Dout = Controler->Kd * (Controler->Err - Controler->Last_Err);
	
	abs_limit(&(Controler->Iout),Controler->Integrate_Max);
	Controler->Control_OutPut = Controler->Pout + Controler->Iout + Controler->Dout;
	abs_limit(&(Controler->Control_OutPut),Controler->Control_OutPut_Limit);
