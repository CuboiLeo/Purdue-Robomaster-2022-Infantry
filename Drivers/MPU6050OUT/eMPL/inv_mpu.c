 ...
 */
int yu5,yu6,yu7,yu8,yu9,yu10,yu11,yu12;
int mpu_read_fifo_stream(unsigned short length, unsigned char *data,
    unsigned char *more)
{
    unsigned char tmp[2];
    unsigned short fifo_count;
	yu5 = !st.chip_cfg.dmp_on;
    //if (!st.chip_cfg.dmp_on)
      //  return -1;
		yu6 = !st.chip_cfg.sensors;
    //if (!st.chip_cfg.sensors)
      //  return -1;
yu7 = i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, tmp);
...
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	unsigned char more;
	long quat[4];
  yu1 = dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors,&more);	
	...
	**/
	...
	//{
		...
		*pitch = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3;	// pitch
		pitch_angle = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3;
